      ...
      <div className="container-lunex py-12 md:py-20">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-10 lg:gap-16">
          <RevealOnScroll className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:sticky lg:top-24 h-fit">
            {(product.images.length ? product.images : ["x"]).map((img, i) => (
                <div
                    key={i}
          ...
          <RevealOnScroll delay={0.08} className="w-full lg:max-w-lg text-left">
            {product.isNew && (
            ...
            <p className="text-lg text-lunex-mute mb-4">CHF {product.priceCHF}.00</p>

            <p className="text-base text-lunex-mute leading-relaxed mb-8">{product.description}</p>
            ...
            <details className="border-t border-lunex-border group text-left">
              <summary className="flex items-center justify-between cursor-pointer text-sm uppercase tracking-[0.2em] py-2">
                Care
                <span className="text-lunex-mute group-open:rotate-45 transition-transform">+</span>
              </summary>
              <ul className="space-y-1.5 text-base text-lunex-mute mb-1.5">
        ...
        {related.length > 0 && (
            <section className="mt-24 md:mt-32 border-t border-lunex-border pt-16">
              <h2 className="font-display text-3xl md:text-4xl uppercase mb-10 text-center lg:text-left">You may also like</h2>
